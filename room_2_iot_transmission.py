...
table = dynamodb.Table("BedRoom2Table")
def send_to_dynamodb(data):
    try:
        table.put_item(Item=data)
        logger.info("Data sent to DynamoDB successfully")
    except Exception as e:
        logger.error(f"Failed to send data to DynamoDB: {str(e)}")
...
def convert_float_to_decimal(obj):
    if isinstance(obj, float):
        return Decimal(str(obj)) # This also prevents precision loss
...
def read_sensor_data():
    ...
    try:
        ...
        logger.debug(f"Temperature: {sensor_data['payload']['temperature']} C, Humidity: {sensor_data['payload']['humidity']}%, Pressure: {sensor_data['payload']['pressure']} hPa")
        return sensor_data
...
try:
    ...
    while True:
        ...
        if sensor_data:
            ...
            dynamo_data = convert_float_to_decimal(sensor_data)
            send_to_dynamodb(dynamo_data)
            sample_count += 1
            if sample_count % 12 == 0:
                logger.info(f"Published to AWS IoT: Temperature: {sensor_data['payload']['temperature']}C, Humidity: {sensor_data['payload']['humidity']}%")
